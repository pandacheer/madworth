<?php echo $head; ?>
<div role="main" class="ui-content dg-productlist"  data-action="<?php echo $doc['seo_url'] ?>" data-offset="<?php echo $doc['offset'] ?>">
    <ol class="breadcrumb">
        <?= $breadcrumb ?>
    </ol>
        ...
        ?>
            <div class="dg-productcell" style="position: relative">
                ...
                <a id="productLink<?php echo $product_id ?>" href="/collections/<?= $doc['seo_url'] ?>/products/<?= $productInfo['seo_url'] ?>">
                    <img alt="<?php echo htmlspecialchars_decode($productInfo['title']) ?>" src="<?php echo IMAGE_DOMAIN . $productInfo['image'] ?>"  class="dg-productcell-image">
                </a>
                <div class="dg-productcell-info">
                    <div class="dg-productcell-title"><a href="/collections/<?= $doc['seo_url'] ?>/products/<?= $productInfo['seo_url'] ?>"><?php echo htmlspecialchars_decode($productInfo['title']) ?></a></div>
...
<script>
var lastPage= <?php echo $currentPage ?>;

fbq('track', 'ViewContent');

    //sorting function
    $('#sort').change(function (e) {
        var url = '/collections/' + $('.dg-productlist').data('action') + '/page/1/' + $('#sort').val() + '/' + $('#tag1').val() + '/' + $('#tag2').val();
        self.location = url;
    });
                ...
                if (result.success) {
                    ...
                    $.each(result.productList, function (i, rowdata) {
                        ...
                        rows = '<div class="dg-productcell">' +
                                ...
                                '<img src="<?php echo IMAGE_DOMAIN ?>' + rowdata.image + '" class="dg-productcell-image">' +
                                '</a>' +
                                '<div class="dg-productcell-info">' +
