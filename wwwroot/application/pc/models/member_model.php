...
class Member_model extends CI_Model {
    ...
    // $postMemberPwd ： 新的密码
    // $postMemberInfo ：新的信息
    function updatePersonal($country_code, $member_id, $change, $postMemberPwd, $postMemberInfo, $member) {
        $this->load->helper('encryption');
        $salt = createSalt();
        $updatePwd = array(
        ...
        if ($change == 2) {//只更新密码
            ...
            if (empty($postMemberPwd)) {
                unset($memberData['member_pwd'], $memberData['member_salt']);
            }
