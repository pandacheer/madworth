	...
	public function addCart($country, $member_email, $arr) {
		...
		if ($cart->count ( array ('_id' => $member_email ) )) {
			...
			if ($cart->count ( $where )) {
				...
				$result = $cart->update ( $where, $data, array (
						"multiple" => true 
				) );
			} else {
				$where = array (
	...
	public function delCart($country, $member_email, $product_sku, $product_bundle, $state) {
		...
		if ($state == 1) {
			if ($product_bundle == 1) {
				...
				$diyimgarr = $cart->findOne($where,array('info.img_url'=>1,'_id'=>0));
                                if(isset($diyimgarr['info'][0]['img_url'])&&$diyimgarr['info'][0]['img_url']!=''){
                                    @unlink($_SERVER['DOCUMENT_ROOT'] . '/../uploads'.$diyimgarr['info'][0]['img_url']);
                                    $unsysdiyimg = $this->CI->mongo->selectCollection ('SYS_diyimg');
                                    $unsysdiyimg->remove(array('imgurl'=>$diyimgarr['info'][0]['img_url']));
                                }
				$result = $cart->update ( $where, $data );
			}
		} else {
			$where = array (
					'_id' => $member_email 
                        ...
                        if(!empty($diyimgarr)){
                            $unsysdiyimg = $this->CI->mongo->selectCollection ('SYS_diyimg');
                            foreach($diyimgarr['info'] as $value){
