                ...
                if ($products) {
                    ...
                    foreach ($products as $key => $value) {
                        if ($value ['status'] != 1 || $value ['product_DetailsStatus'] != 1
                                ||(!$value['img_url']&&isset($value['diy'])&&$value['diy']==1)
                                ||($value['img_url']&&(!isset($value['diy'])||$value['diy']==0))) {
                            unset($products [$key]);
                            $this->cart_model->delCart($this->country, $this->member_email, $value ['product_skudiy'], 1, 1);
                        } else {
...
//echo $this->page['collection_offer'];exit;
    ...
    public function addCart() {
        if ($this->input->is_ajax_request()) {
            ...
            $isdiy = $this->product_model->isDiy($product_sku);
            if($isdiy){
                $product_attr = $product_attr!='' ? $product_attr."/".time() : time();
                $product_diyimg = $this->input->post('p_diyImg', TRUE);
                
                if(!$product_diyimg||!file_exists($_SERVER['DOCUMENT_ROOT'] . '/../uploads'.$product_diyimg)){
                    $this->addCart_result(5);
                    exit;
                }
            }else{
                $product_diyimg = '';
            ...
            switch ($product_type) {
                case 1 :
                    $this->_addCart_1($product_type, $product_id, $product_sku, $product_qty, $product_attr,$product_diyimg);
                    break;
                case 2 :
                    $this->_addCart_2($product_type, $product_id, $product_sku, $product_qty, $product_attr,$product_diyimg);
                    break;
                case 3 :
                    $this->_addCart_3($product_type, $product_id, $product_sku, $product_qty, $product_attr,$product_diyimg);
                    break;
                default :
                    break;
            }
    ...
    public function _addCart_1($product_type, $product_id, $product_sku, $product_qty, $product_attr,$product_diyimg='') {
        if ($product_attr) {
        ...
        if ($status) {
            ...
            if ($this->member_email) {
                $this->addCart_result($this->cart_model->addCart($this->country, $this->member_email, $data), $product_id,$product_diyimg);
            } else {
                $this->addCart_result($this->addCartCookie($data), $product_id,$product_diyimg);
            }
        } else {
    ...
    public function _addCart_2($product_type, $product_id, $product_sku, $product_qty, $product_attr,$product_diyimg='') {
        $data = array(
            'product_id' => $product_id,
            'bundle_type' => (int) $product_type,
        ...
        if($product_diyimg){
            $sku = substr($data ['product_sku'],0,strrpos($data ['product_sku'],'/'));
        }else{
            $sku = $data ['product_sku'];
        }
        if ($this->product_model->exist($this->country, $product_id, $sku, 2)) {
            if ($this->member_email) {
                $this->addCart_result($this->cart_model->addCart($this->country, $this->member_email, $data),0,$product_diyimg);
            } else {
                $this->addCart_result($this->addCartCookie($data),0,$product_diyimg);
            }
        } else {
            $this->addCart_result(0);
        }
    ...
    public function _addCart_3($product_type, $product_id, $product_sku, $product_qty, $product_attr,$product_diyimg='') {
        ...
        if($product_diyimg){
            $sku = $product_sku . '/' . $bundle_attr [0] . ',' . $product_sku . '/' . $bundle_attr [1];
            $data['product_sku'] = $product_sku . '/' . $bundle_attr [0] . ',' . $product_sku . '/' . $bundle_attr [1]. ',' . $product_sku . '/' . $bundle_attr [2];
        }else{
            $sku = $data ['product_sku'];
        }
        if ($this->product_model->exist($this->country, $product_id, $sku, 3)) {
            if ($this->member_email) {
                $this->addCart_result($this->cart_model->addCart($this->country, $this->member_email, $data),0,$product_diyimg);
            } else {
                $this->addCart_result($this->addCartCookie($data),0,$product_diyimg);
    ...
    public function addCart_result($state = 0, $product_id = 0,$diyImg = '') {
        if ($state) {
            if ($state === 3) {
                if($diyImg){
                    @unlink($_SERVER['DOCUMENT_ROOT'] . '/../uploads'.$diyImg);
                }
                exit(json_encode(array(
                    'success' => false,
                    'resultMessage' => lang('cart_NumberError')
                )));
            } else if ($state === 4) {
                if($diyImg){
                    @unlink($_SERVER['DOCUMENT_ROOT'] . '/../uploads'.$diyImg);
                ...
                exit(json_encode(array(
                    'success' => false,
                    'resultMessage' => lang('cart_statusError')
                )));
            }else if($state === 5){
                if($diyImg){
                    @unlink($_SERVER['DOCUMENT_ROOT'] . '/../uploads'.$diyImg);
                ...
                exit(json_encode(array(
                    'success' => false,
                    'resultMessage' => lang('cart_diyImgError')
                )));
            } else {
                // 加入站点加入购物车次数
                ...
                if($diyImg){
                    ...
                    $subscription->update(array('imgurl' =>$diyImg),array('$set'=>array('cart'=>intval(date('Ymd')))));
                    $token = time() . mt_rand(1, 9999999999);
                    exit(json_encode(array(
                        'success' => true,
                        'resultMessage' => lang('cart_Success'),
                        'uploadUrlParm'=>array('token'=>$token,'tokenCheck'=>md5($token . '305046350@qq.com'))
                    )));
                }
                exit(json_encode(array(
                    'success' => true,
    ...
    public function delCart() {
        if ($this->input->is_ajax_request()) {
            ...
            if ($this->member_email) {
                ...
                if ($result) {
                    exit(json_encode(array('success' => true, 'discountNumber' => $this->getDiscount())));
                }
            } else {
                ...
                if ($arr) {
                    ...
                    if ($state == 1) {
                        ...
                        if(isset($img_url)&&$img_url!=''){
                            @unlink($_SERVER['DOCUMENT_ROOT'] . '/../uploads'.$img_url);
                            $unsysdiyimg = $this->mongo->selectCollection ('SYS_diyimg');
                            $unsysdiyimg->remove(array('imgurl'=>$img_url));
                        }
                        $this->input->set_cookie("cart", serialize($products), 864000);
                        $_COOKIE['cart'] = serialize($products);
                        exit(json_encode(array(
                        ...
                        if(!empty($products)){
                            ...
                            foreach ($products as $value){
                                if(isset($value['img_url'])&&$value['img_url']!=''){
                                    @unlink($_SERVER['DOCUMENT_ROOT'] . '/../uploads'.$value['img_url']);
                                    $unsysdiyimg->remove(array('imgurl'=>$value['img_url']));
                                }
                            }
                        }
                    }
