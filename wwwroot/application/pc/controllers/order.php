...
class order extends MY_Controller {
    ...
    public function __construct() {
        ...
        $this->load->model('template_model'); //多加
...
//        print_r($cartOrderInfo);exit;
    ...
    function _ppCreditCardCreateOrder($actionID, $actionEmail, $actionName, $orderNumber, $cartOrderInfo, $orderGuestbook, $addressInfo, $insertBillInfo, $shippingInfo) {
        ...
        $insertOrderInfo = array(
            ...
            'transaction_id' => 0,
            'diy' => $cartOrderInfo['diy'],//是否含有DIY产品
    ...
    function _ppCreditCardPayment($currencyCodeType, $logIn, $actionID, $actionEmail, $orderInfo, $bill_address_id, $address_id, $creditCardInfo) {
        ...
        $address1 = urlencode($orderInfo['OrderBillInfo']['receive_add1']);
        $address2 = urlencode($orderInfo['OrderBillInfo']['receive_add2']);
        $city = urlencode($orderInfo['OrderBillInfo']['receive_city']);
        $state = urlencode($orderInfo['OrderBillInfo']['receive_province']);
        $zip = urlencode($orderInfo['OrderBillInfo']['receive_zipcode']);
...
//        $url = dirname('https://' . $serverName . $this->input->server('REQUEST_URI'));
        ...
        if ($ack == "SUCCESS") {
            ...
            $resArray['actionEmail'] = $actionEmail;
            $payLogInfo = array(
                'transactionid' => 'error',
...
//            redirect("home/showError/C" . $resArray['L_ERRORCODE0']);
//            redirect("home/showError/P1100");
        }
    }
...
//        $url = dirname('https://' . $serverName . $this->input->server('REQUEST_URI'));
        ...
        $order_amount = round(($cartOrderInfo['payment_amount'] - $cartOrderInfo['freight_amount']) / 100, 2); //商品总价+包装费+保险费
...
//                $haveTotal = $this->product_model->checkOutTotal($this->country, $product['product_id'], $haveTotal); //统计当天产品ChectOut次数
                ...
                switch ($product['bundle_type']) {
                    case 1:
                        ...
                        $orderInfo['diy']=$orderInfo['diy']||$product['diy'];
                        ++$index;
                        break;
                    case 2:
                        $productList[$index] = array(
...
////        exit;
        if ($this->session->userdata('orderEmail')) {
            ...
            //$product_ids = $this->orderdetails_model->listByOrderNumber($this->country, $this->page['order_id'], 'product_id,product_sku,payment_price,product_quantity');
            $product_ids = $orderCreateSuccess['OrderDetailsInfo'];
            $this->page['share'] = array('title' => '', 'desc' => '', 'image' => '', 'url' => '');
            if (!empty($product_ids)) {
                $pro_info = $this->product_model->findOne($product_ids[0]['product_id']);
                $this->load->model('collection_model');
                $collection = $this->collection_model->getCollectionUrl($this->country, $product_ids[0]['product_id']);
                $this->page['share'] = array('title' => $pro_info['title'], 'desc' => $pro_info['seo']['description'], 'image' => IMAGE_DOMAIN . $pro_info['image'], 'url' => site_url('collections/' . $collection . '/products/' . $pro_info['seo_url']));
            }
            $ga_addItem = '';
            $itemTmp = '';
...
//            echo $ga_addItem . "<br><br>";
//            echo $ga_addTransaction . '<br><br>';
//            exit;
