	...
	public function getProduct() {
		...
		if ($this->input->is_ajax_request ()) {
			...
			if ($countdown_id) {
				...
				if($result ['children'] && is_array ( $countdownInfo ) && $countdownInfo ['status'] == 2){
					foreach($result ['productDetails'] as  $key=>$productDetails){
						unset ($result ['productDetails'][$key]['cost']);
						$result ['productDetails'][$key]['price'] = $this->countdown_model->getPrice ( $countdown_id, $productDetails ['price'] );
					}
				}
			}
