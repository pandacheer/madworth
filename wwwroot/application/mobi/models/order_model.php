...
class order_model extends CI_Model {
    ...
    //获取前台订单
    public function getOrder($country, $member_id, $fields = 'order_id,order_number,member_id,member_email,member_name,order_quantity,order_amount,payment_amount,offers_amount,coupons_id,order_insurance,order_giftbox,freight_amount,receive_name,create_time,send_status,is_resend,pay_status,doc_status,update_time,pay_type,estimated_time,transaction_id,operator') {
    ...
    //根据order_id获取订单信息
    function getInfoByID($country_code, $order_id, $fields = 'order_id,order_number,member_id,member_email,member_name,order_quantity,order_amount,payment_amount,offers_amount,coupons_id,freight_amount,receive_name,create_time,send_status,is_resend,pay_status,doc_status,update_time,pay_type,estimated_time,transaction_id,operator') {
        $sql = "select {$fields} from {$country_code}_order where order_id={$order_id}";
        return $this->db->query($sql)->row_array();
    }

    //根据order_number获取订单信息
    function getInfoByNumber($country_code, $order_number, $fields = 'order_id,order_number,member_id,member_email,member_name,order_quantity,order_amount,payment_amount,offers_amount,coupons_id,freight_amount,receive_name,create_time,send_status,is_resend,pay_status,doc_status,update_time,pay_type,estimated_time,transaction_id,operator') {
        $sql = "select {$fields} from {$country_code}_order where order_number={$order_number}";
        return $this->db->query($sql)->row_array();
    }
    ...
    //支付成功后的操作
    function payment($country_code, $member_id, $member_email, $coupons_id, $orderNumber, $paymentAmount, $transaction_id, $orderCreateTime, $address_id = 0) {
        $this->db->trans_start();
        //更新订单状态和支付交易号
        ...
        if ($this->db->trans_status()) {
            //更改销售数量
            $sql = 'select product_id,product_sku,total_qty from ' . $country_code . '_order_details where order_number=' . $orderNumber;
