...
class coupons_model extends CI_Model {
    ...
    function getMyCoupons($country, $member_email) {
        //从优惠券主表查找有效的公有优惠券
        $myCoupons = array();
        $publicSql = "SELECT coupons_id,type,amount,frequency,end,`condition`,min,max FROM {$country}_coupons WHERE display=1 and private = 2 AND status = 2 AND start < ? AND end > ?";
        $pubicWhere = array(
            'start' => time(),
            'end' => time()
        );
