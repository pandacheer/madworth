...
//        exit;
    ...
    //根据产品id获取绑定的商品和喜欢的商品
    public function specialProduct($country, $array, $limit = 0) {
        (int) $limit < 1 && $limit = 5;
        $product = $this->CI->mongo->selectCollection($country . '_product');
        $result = $product->find(array('_id' => array('$in' => $array)))->sort(array("sold.total" => -1))->limit($limit);
    ...
    public function cartPro($country, $data) {
        ...
        if ($data) {
            foreach ($data as $key => $vo) {
                ...
                $pro = $this->pro->findOne(array('_id' => $id), array('title' => 1, 'children' => 1, 'seo_url' => 1, 'sku' => 1, 'image' => 1, 'price' => 1, 'bundle' => 1, 'weight' => 1, 'status' => 1, 'freebies' => 1));
...
//        $data ['weight_amount'] = $vo ['product_qty'] * $data ['product_weight'];
    ...
    function getRcommend($country = 'US', $pro_list = array()) {
        ...
        $specialProduct = $this->specialProduct($country, $productId, 2);
