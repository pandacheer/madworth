...
class collections extends MY_Controller {
    ...
    function __construct() {
        ...
        $this->collectionMap = array(
            ...
            'get-ready-for-winter-men' => [
                'new' => 'Mens-Fashion',
                'tag' => 1
            ],
            'get-ready-for-winter-women' => [
                'new' => 'Womens-Fashion',
                'tag' => 1
            ],
            'aircraft-cup' => [
            ...
            'assassin-s-creed-theme-day-grab' => [
                'tag' => 0
            ],
            'compression-tights' => [
                'tag' => 0
...
//            $filter2 = urldecode($filter2);
            ...
            $this->page ['doc'] = $collection->findOne(array(
                'seo_url' => new MongoRegex('/^' . $seo_url . '$/i')
            ));
            if (!is_array($this->page ['doc'])) {
                redirect('home/showError404');
            }
            if (isset($this->page ['doc']['description']) && ($this->page ['doc']['description'] == '<p><br></p>' || $this->page ['doc']['description'] == htmlspecialchars('<p><br></p>'))) {
                $this->page ['doc']['description'] = '';
            } elseif (!isset($doc['description'])) {
                $this->page ['doc']['description'] = '';
            }
            if (isset($this->page ['doc']['description2']) && ($this->page ['doc']['description2'] == '<p><br></p>' || $this->page ['doc']['description2'] == htmlspecialchars('<p><br></p>'))) {
                $this->page ['doc']['description2'] = '';
            } elseif (!isset($doc['description2'])) {
                $this->page ['doc']['description2'] = '';
            }
            $a = A(array(
                $seo_url,
                $this->page ['doc'] ['seo_title']
                    ), $this->page ['original_menu']);
            $this->page ['title'] = $this->page ['doc'] ['seo_title'];
            $this->page ['description'] = $this->page ['doc'] ['seo_description'];
            $this->page ['keywords'] = $this->page ['doc'] ['seo_keyword'];
            if ($currentPage > 1) {
            ...
            if ($this->page ['doc'] ['show_comment'] == 2) {
                $this->load->model('comment_model');
                $this->page ['collectionComment'] = $this->comment_model->getInfoByCollectionId($this->page ['country'], $this->page ['doc'] ['_id']);
            } else {
            ...
            // $template = 'product_list_col' . $this->page['doc']['columns'];
            // $this->page['breadcrumb'] = '<li><a href="/">DrGrab</a></li><li> > </li><li class="active"><a href="/collections/' . $this->page['doc']['seo_url'] . '" style="color:#00B6C6">' . $this->page['doc']['title'] . '</a></li>';
            $this->page ['breadcrumb'] = '<li><a href="/">DrGrab</a></li>';
            if (!empty($a)) {
                foreach ($a as $ke => $va) {
                    $class = count($a) - 1 == $ke ? ' class="active"' : '';
                    $this->page ['breadcrumb'] .= ' > <li' . $class . '><a href="' . $va [1] . '" style="color:#00B6C6">' . $va [0] . '</a></li>';
                }
            }
            $offset = ($currentPage - 1) * $this->prePage;
            $this->page ['doc'] = $this->getPageProduct($this->page ['country'], $this->page ['doc'], $filter1, $filter2, $offset);
            ...
            $product_listView = $this->template_model->getStyle($this->terminal, $this->page ['country'], 'product_list');
            $this->load->view($product_listView, $this->page);
        }
    }
    ...
    function getPageProduct($coturny_code, $doc, $filter1, $filter2, $offset) {
        ...
        if ($doc ['model'] == 1) { // 手动模式
            ...
            if (count($doc ['conditions']) == 1) {
                $mongoCondtion = array(
                    $doc ['conditions'] [0] ['fields'] => $doc ['conditions'] [0] ['values']
                );
            } else {
                $mongoCondtion = [];
                foreach ($doc ['conditions'] as $condition) {
                    if ($condition ['fields'] == 'type') {
                        $mongoCondtion [] = array(
                            $condition ['fields'] => (int) $condition ['values']
                        );
                    } else {
                        if ($condition ['link'] == 'contains') {
                            $mongoCondtion [] = array(
                                $condition ['fields'] => new MongoRegex("/{$condition['values']}/")
                            );
                        } else {
                            $mongoCondtion [] = array(
                                $condition ['fields'] => $condition ['values']
                            );
                        }
                    }
                }
    ...
    function loadPage() {
        ...
        if (count($doc ['allow']) > 0) {
            exit(json_encode(array(
                'success' => TRUE,
                'productList' => $doc ['allow'],
                'offset' => $doc ['offset'],
                'collection_seo' => $seo_url,
    ...
    public function _products($collection, $pro_url) {
        $this->load->helper('form');
        ...
        $this->page ['canonical'] = site_url('collections/' . $this->page ['return'] ['seo_url'] . '/products/' . $pro_url);
        // 组装SEO信息
        ...
        $this->page ['head'] = $this->load->view($headView, $this->page, true);
        $a = A(array(
            $this->page ['return'] ['seo_url'],
            $this->page ['return'] ['seo_title']
                ), $this->page ['original_menu']);
        // 组装breadcrumb
        // $this->page['breadcrumb'] = '<li><a href="/" >DrGrab</a></li><li> > </li><li><a href="/collections/' . $this->page['return']['seo_url'] . '" style="color:#00B6C6">' . $this->page['return']['title'] . '</a></li>';
        $this->page ['breadcrumb'] = '<li><a href="/">DrGrab</a></li>';
        if (!empty($a)) {
            foreach ($a as $ke => $va) {
        ...
        $this->page ['desc_shipping'] = $this->page_model->_findSeo($this->country, 'Product-Tab-Shipping-mobile');
