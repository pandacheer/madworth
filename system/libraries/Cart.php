...
class CI_Cart {
	...
	function _insert($items = array())
	{
		...
		if ( ! isset($items['sku']) OR ! isset($items['qty']) OR ! isset($items['price']) )
		{
		...
		if ( ! preg_match("/^[".$this->product_id_rules."]+$/i", $items['sku']))
		{
		...
		{
			log_message('error', 'An invalid price was submitted for product ID: '.$items['sku']);
			return FALSE;
		}
		...
		{
			$rowid = md5($items['sku'].implode('', $items['options']));
		}
		else
		{
